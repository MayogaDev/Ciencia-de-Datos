...
function createGeoDistributionMap() {
    ...
    d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(function(geojson) {
        ...
        // Agregar puntos para ubicaciones (utilizando OpenCage para obtener latitud y longitud)
        locationArray.forEach(async (d) => {
            const coordinates = await getCoordinates(d.location); // Obtener coordenadas
            if (coordinates) {
